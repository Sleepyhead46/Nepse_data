...
def format_hourly_row(date_str, time_str, row_data):
    """Formats raw scraped row into an hourly record list.
...
def hourly_records_to_dataframe(records):
    """Converts a list of formatted hourly records to a pandas DataFrame."""
    return pd.DataFrame(records, columns=HOURLY_COLUMNS)
